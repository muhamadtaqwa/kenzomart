...
class DuitkuService
{
    ...
    public function createQrisTransaction(Order $order): array
    {
        ...
        $payload = [
            ...
            'email' => !empty($order->customer?->email) ? $order->customer->email : 'noreply@kenzomart.test',
            'phoneNumber' => $order->customer?->phone ?? '081234567890',
            'customerVaName' => !empty($order->customer?->name) ? $order->customer->name : ($order->customer?->phone ?? 'Customer'),
